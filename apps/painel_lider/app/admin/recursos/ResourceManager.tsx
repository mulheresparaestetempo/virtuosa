...
import { createLinkResource, deleteResource, getResources, ResourceItem, ResourceType } from '@/lib/services/resource-service';
...
export default function ResourceManager({ type, title, icon, description, urlPlaceholder }: Props) {
  ...
  const save = async () => {
    ...
    try {
      const created = await createLinkResource(type, name, desc, url, user.uid);
      setItems((current) => [created, ...current]); setName(''); setDesc(''); setUrl('');
  ...
  return <main className="page"><header className="header"><div><div className="brand">{icon} {title}</div><small>Conteúdo persistente no Firebase</small></div></header><section className="main">
    ...
    <div className="card"><h2>Novo {title.slice(0, -1) || title}</h2><div style={{ display: 'grid', gap: 12, marginTop: 18 }}>
      <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Título" style={{ width: '100%', padding: 12, border: '2px solid #E8E8E8', borderRadius: 18, boxSizing: 'border-box', fontFamily: 'inherit', fontSize: '1rem' }} />
      <textarea value={desc} onChange={(e) => setDesc(e.target.value)} placeholder="Descrição (opcional)" rows={4} style={{ width: '100%', padding: 12, border: '2px solid #E8E8E8', borderRadius: 18, boxSizing: 'border-box', resize: 'vertical', fontFamily: 'inherit', fontSize: '1rem' }} />
      <input value={url} onChange={(e) => setUrl(e.target.value)} placeholder={urlPlaceholder || 'Link público'} style={{ width: '100%', padding: 12, border: '2px solid #E8E8E8', borderRadius: 18, boxSizing: 'border-box', fontFamily: 'inherit', fontSize: '1rem' }} />
